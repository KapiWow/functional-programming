...
;role in all branches of engineering design. It is particularly important
;to computer programming, because in programming not only
;can we formulate new languages but we can also implement these languages
;by constructing evaluators. An evaluator (or interpreter) for a
;programming language is a procedure that, when applied to an expression
;of the language, performs the actions required to evaluate that expression.
...
(next-slide "вспомогательные функции")
...
;на вход мы получаем список выражений, которые надо рассчитать и окружение
;вычисляем первый аргумент, к остальным применяем list-of-values к остальным аргументам

;написать, чтобы считалось справа налево и слева направо


(next-slide "conditions")
...
(define (eval-definition exp env)
  (define-variable! (definition-variable exp)
                    ...
                    env)
  'ok)
;define-variable! будет позже


(next-slide "Присвоение")
...
(define (tagged-list? exp tag)
  (if (pair? exp)
    (eq? (car exp) tag)
    false))
;присвоить значение окружению и переопределить значение переменной


(next-slide "Expressions")
