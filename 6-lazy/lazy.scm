...
; Тоже будет ошибка
;Это связано с порядком вычилсения выражений.  Scheme использует
...
(list-ref (solve (lambda (x) x) 1 0.001) 500)
))

;(use-modules (system vm vm))

;(debug-set! stack 10485760)
;(debug-set! stack 10485760)
(println (debug-options))
;(call-with-vm (the-vm) (run-prog prog))
(run-prog prog)
